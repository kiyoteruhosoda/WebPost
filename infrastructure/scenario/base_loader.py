# infrastructure/scenario/base_loader.py
...
class ScenarioLoaderBase(ABC):
    def load_from_file(self, path: str | Path) -> Scenario:
        scenario_path = Path(path)
        if not scenario_path.exists():
            raise ScenarioLoadError(f"Scenario file not found: {scenario_path}")

        data = self._load_file(scenario_path)
        if data is None:
            raise ScenarioLoadError(f"Scenario file is empty: {scenario_path}")
        if not isinstance(data, dict):
            raise ScenarioLoadError(f"Scenario file is invalid: {scenario_path}")
